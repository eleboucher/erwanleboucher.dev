...
date: 2026-04-21
description: How Meta scrapes everything to train its AI model, and why that's a problem.
posted: true
---

After recent issues with GitHub, I decided to host my own Forge server. This let me be the only one responsible if my git server goes down. It was running fine; I moved everything onto a Forgejo runner for my CI/CD, with my own runner backend for Kubernetes (I'll write about that). Everything was running smoothly, until it wasn't.

One day, Bernd (aka bjw-s), a fellow homelabber, creator of the mythic helm-template used by many, even OVH, told me his server was being spammed by requests. We're talking hundreds of hits per minute from AI crawlers, hammering the server hard enough to degrade performance and rack up bandwidth. Not ideal when you're running this on your own hardware at home.
...
Our first reaction was to use Anubis. It's a tool that sits in front of every request and challenges anything that doesn't look legitimate, a great defense against bots and scrapers.
...
I needed to block things further upstream, at the VPS level, before they even reached my home. So I decided to go nuclear: block most AI scrapers and ban any traffic from GAFAM (Google, Apple, Facebook, Amazon, Microsoft).

Since we already had CrowdSec running on our VPS alongside [Pangolin](/blog/pangolin), the fix was to push the blocking logic there. First thing I noticed: the generated Docker Compose file from Pangolin had CrowdSec running in test mode, so it was doing absolutely nothing:

```yaml title="docker-compose.yml" "-t"
  crowdsec:
    container_name: crowdsec
    command: -t # test mode: validates the config and exits, enforcing nothing
```
