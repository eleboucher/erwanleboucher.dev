...
Since we already had CrowdSec running on our VPS alongside Pangolin [link of the article](https://erwanleboucher.dev/\#/blog/pangolin). the fix was to push blocking logic there. First thing I noticed: the generated Docker Compose file from Pangolin had CrowdSec running in test mode — so it was doing absolutely nothing:
```yaml
crowdsec:
